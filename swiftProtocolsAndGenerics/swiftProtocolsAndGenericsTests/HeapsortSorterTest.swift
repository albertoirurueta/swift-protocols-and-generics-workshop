...
class HeapsortSorterTest: XCTestCase {
    ...
    func testSortingMethod() {
        let sorter = HeapsortSorter()
        XCTAssertEqual(sorter.method, SortingMethod.heapsort)
    }
    
    func testSortWithRange() {
        for _ in 1...HeapsortSorterTest.times {
            let length = HeapsortSorterTest.minLength +
                Int(arc4random_uniform(UInt32(
                    HeapsortSorterTest.maxLength -
                        HeapsortSorterTest.minLength)))
            ...
            for i in 0..<length {
                array[i] = HeapsortSorterTest.minValue +
                    Int(arc4random()) % (HeapsortSorterTest.maxValue -
                        HeapsortSorterTest.minValue)
            }
